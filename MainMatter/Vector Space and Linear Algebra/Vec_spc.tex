...
\end{theorem}
\begin{proof}
       Let $u_1,\cdots, u_m$ be a linearly independent list. Consider adding $u_1$ to $v_1,\cdots, v_n$ and by \lemmaref{lem: removing_from_spanning} we know $u_1,v_1,\cdots, v_n$ is linearly dependent and 
...
\begin{lemma}[Steinitz] \namedlabel{lem: steinitz}{the Steinitz exchange lemma}
...
\end{lemma}
\begin{proof}
    For the first part, if $v_1, \dots, v_m$ is linearly independent, then it is a basis. Otherwise, by \lemmaref{lem: removing_from_spanning}, we can remove one vector without changing the spanned space. Continuing this process, we will eventually obtain a linearly independent list that spans $V$, which is a basis.
...
\end{theorem}
\begin{proof}
    It's done with \lemmaref{lem: steinitz}.
\end{proof}

We close this subsection with the following results concerning operations of vector spaces and their dimension.
\begin{theorem}[the Dimension Inequality]
    For vector space $V$ and its subspace $U$, suppose $U \subset V$. Then $0 \leq \dim U \leq \dim V$ and it becomes an equation only when $U$ is trivial or $U=V$.
\end{theorem}

\begin{theorem}For vector space $V$ and $U$,
    $$\dim (U+V) = \dim U +\dim V -\dim(U\cap V).$$
\end{theorem}

\begin{theorem}
    Let $V_1,\cdots,V_m$ be vector spaces, then $V_1 +\cdots +V_m$ is a direct sum $\iff \dim (V_1 +\cdots+V_m)=\dim V_1 +\cdots +V_m$.  
\end{theorem}