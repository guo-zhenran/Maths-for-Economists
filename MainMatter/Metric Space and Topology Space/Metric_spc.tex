...
\begin{example}[the Discrete and Natural Metric]
    ...
    \,where $\delta_{x,y}$ is the Dirichlet notation, meaning 
...
However, we can't find a suitable limit for $b_n$. Thus, we're to extend the metric on $\R$ so that sequences like $b_n$ can be given a limit.
\begin{definition}[Extended Concepts Related to $\pm \infty$]
    ...
    \begin{description}
        ...
        \item[Convergence to $\pm \infty$] $x_n\to \pm \infty (n\to \infty)$ is defined as: $$\forall \varepsilon>0 : \exists K\in \R^+ \text{ such that } |x_n|\geq \varepsilon, \forall n >K.$$
    \end{description} 
\end{definition}
